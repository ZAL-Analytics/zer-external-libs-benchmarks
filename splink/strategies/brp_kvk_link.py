...
Uses soundex_initial_year + postcode exact-match blocking (no year_month).
Dutch postcodes are street-level (~15-30 households), making postcode a strong
OR-rule for cross-source matching where surname Soundex or first-initial may
differ due to abbreviated voornamen in KvK records.
"""
...
def build(dfs, link_type="dedupe_only"):
    ...
    Uses soundex_initial_year + postcode blocking.
    """
    ...
    if _POSTCODE_COL in ref_df.columns:
        blocking_rules.append(block_on(_POSTCODE_COL))
