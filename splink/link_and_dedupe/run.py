...
def main():
    ...
    comparisons, blocking_rules, em_col, surname_col = build(
        toml_data, dfs, link_type="link_and_dedupe")

    if comparisons is None:
        _ADDR_KEYS = ("straat", "adres", "street", "address", "city", "place", "woon")
        name_cols = [c for c in dfs[0].columns if any(k in c.lower() for k in ("naam", "name", "nomen", "alias"))
                     and not any(k in c.lower() for k in _ADDR_KEYS)]
        date_cols = [c for c in dfs[0].columns if any(k in c.lower() for k in ("datum", "date", "dob", "birth"))]
        addr_cols = [c for c in dfs[0].columns if any(k in c.lower() for k in _ADDR_KEYS)]
        id_cols   = [c for c in dfs[0].columns if any(k in c.lower() for k in ("id", "nummer", "bsn", "number", "code", "postcode"))]
        dfs = [add_blocking_keys(df, name_cols, date_cols, addr_cols, id_cols) for df in dfs]
        comparisons = []
        ...
        if not comparisons:
            for c in list(dfs[0].select_dtypes("object").columns)[:4]:
                comparisons.append(cl.ExactMatch(c))
        bk_cols = [c for c in dfs[0].columns if c.startswith("_bk_")]
        blocking_rules = [block_on(c) for c in bk_cols] or [block_on(dfs[0].columns[0])]
    ...
    # splink UNION ALL requires all frames to share identical columns.
    ...
    if em_col and em_col in dfs[0].columns:
        linker.training.estimate_parameters_using_expectation_maximisation(block_on(em_col))
    if surname_col and surname_col in dfs[0].columns and surname_col != em_col:
    ...
    run_id = f"splink_link_and_dedupe_{dataset_name}_{ts}"
    ...
    with open(out_dir / csv_path, "w", newline="") as f:
        ...
        w.writerow(["splink","link-and-dedupe",dataset_name,run_id,ts_iso,
    ...
    print(f"[splink/link_and_dedupe] written: {json_path}")
    print(f"[splink/link_and_dedupe] records={total_records}  pairs={candidate_pairs}  elapsed_ms={elapsed_ms}")
    if prec is not None:
        print(f"[splink/link_and_dedupe] precision={prec:.3f}  recall={rec:.3f}  f1={f1_v:.3f}  opt_thr={opt_thr}  pr_auc={pr_auc}  blocking_recall={blk_rec}")
